...
import {User, Home, Mail, Phone, LocateFixed, CheckCircle2, MapPin, Navigation, Edit3, X} from 'lucide-react-native';
...
const SellerRegister = () => {
        ...
        switch (currentStep) {
            case 1: {
                if (!fullName || !email || !phone) {
                    showAlert('Incomplete Form', 'Please fill out all the details');
                    return;
                }
                if (phone.length !== 10) {
                    showAlert('Phone Number', 'Please enter a 10 digit phone number');
                    return;
                }
                const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                if (!emailRegex.test(email)) {
                    showAlert('Email', 'Please enter a valid email address');
                    return;
                }
                if (currentStep < 4) {
                    fadeAnim.setValue(0);
    ...
    const handleMapDrag = (e) => {
        ...
        // Update map region without triggering address update immediately
        setMapRegion(prev => ({
            ...prev,
            latitude,
            longitude,
        ...
        // Debounce the address update to prevent excessive API calls and re-renders
        updateTimerRef.current = setTimeout(() => {
            updateAddressFromPin(latitude, longitude);
        }, 800); // Update address 800ms after user stops dragging
    };
    ...
    const renderPersonalInfo = () => (
        ...
        >
            ...
            <Text style={styles.sectionSubtitle}>We need some basic information to get started</Text>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            >
                ...
                >
                    ...
                    <View style={styles.formCard}>
                        ...
                        {currentStep === 3 && renderHouseDetails()}
            ...
            <SuccessModal
                title={'Registered'}
                subtitle={'Registered Successfully, Redirecting to Dashboard'}
                onAnimationComplete={() => {
